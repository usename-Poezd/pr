...
Невалидный JSON или данные: `400` и `{"error":"invalid request"}`. Поле вопроса `multiple` необязательно и по умолчанию равно `false`. `results_visible` по умолчанию равно `true`, сохраняя публичные результаты для старых клиентов. Если флаг равен `false`, публичные результаты возвращают `403`, а admin-токен всегда даёт доступ. Ограничения: название 1–500 символов, 1–50 вопросов, текст вопроса 1–1000 символов, 2–50 вариантов на вопрос, вариант до 500 символов; варианты после trim не должны повторяться без учёта регистра.
...
`GET /api/polls/{id}/results` доступен публично без query-параметров, если `results_visible` равен `true`. Если `results_visible` равен `false`, возвращается `403`; валидный `admin_token` всегда даёт доступ. Если передан `admin_token`, он проверяется:
